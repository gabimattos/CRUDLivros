...
class Author{
    ...
    static public function create(\Request $request):Author{
        $pdo = \DB::connect();
        $stm = $pdo->prepare("INSERT INTO author (`name`,`surname`) VALUES (?,?)");
    ...
    static public function all():array{
        $pdo = \DB::connect();
        $stm = $pdo->prepare("Select `aid`,`name`,`surname` from author");
        $stm->execute();
    ...
    static public function find($aid):Author{
        $pdo = \DB::connect();
        $stm = $pdo->prepare("Select * from author where aid=?");
    ...
    static public function delete(\Request $request):int{
        $pdo = \DB::connect();
        $stm = $pdo->prepare("Delete from author where id=?");
